...
import type { UITheme, EditorTheme } from './types';
	...
	useEffect(() => {
		...
		if (extended) {
			...
			if (extended.score) {
				...
				root.style.setProperty('--alphatab-secondary-glyph', extended.score.secondaryGlyph);
				root.style.setProperty('--alphatab-staff-line', extended.score.staffLine);
				root.style.setProperty('--alphatab-bar-separator', extended.score.barSeparator);
