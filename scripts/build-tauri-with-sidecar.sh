...
(cd "${ROOT_DIR}" && pnpm tauri build "$@")
install_runtime_files "${ROOT_DIR}/src-tauri/target/release"
shopt -s nullglob
for macos_dir in "${ROOT_DIR}"/src-tauri/target/release/bundle/macos/*.app/Contents/MacOS; do
	install_runtime_files "${macos_dir}"
done
shopt -u nullglob
