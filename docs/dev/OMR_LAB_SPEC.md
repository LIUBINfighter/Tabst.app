  ...
  "messages": [
    ...
    },
    {
      ...
      "content": [
        ...
        {
          "type": "text",
          "text": "Convert this guitar tab to alphaTex format."
        }
      ]
    }
  ],
...
- HTTP 4xx/5xx → 返回 `omr-request-failed` + status code
...
curl -L "https://github.com/ggml-org/llama.cpp/releases/download/${VERSION}/llama-${VERSION}-bin-macos-arm64.zip" \
  -o "/tmp/llama-macos-arm64.zip"
unzip "/tmp/llama-macos-arm64.zip" -d "src-tauri/binaries/"
mv "src-tauri/binaries/llama-server" "src-tauri/binaries/llama-server-aarch64-apple-darwin"
chmod +x "src-tauri/binaries/llama-server-aarch64-apple-darwin"

# macOS x64 (Intel，备用)
curl -L "https://github.com/ggml-org/llama.cpp/releases/download/${VERSION}/llama-${VERSION}-bin-macos-x64.zip" \
  -o "/tmp/llama-macos-x64.zip"
unzip "/tmp/llama-macos-x64.zip" -d "src-tauri/binaries/"
mv "src-tauri/binaries/llama-server" "src-tauri/binaries/llama-server-x86_64-apple-darwin"
...
**推荐 Model ID**: `Qwen/Qwen3.5-VL-0.6B-Instruct`

> **注意**：该模型 ID 为推荐选择，实现前需确认其在 Hugging Face 上的可用性。如不可用，可替换为任意支持 vision 的 0.5B-1B 级别模型（需支持 llama.cpp GGUF 转换）。

**⚠️ 预实现准备（Phase 0 Gate）**：

在开始编码前，必须完成以下准备工作并生成**模型清单文件（Model Manifest）**：
