# 📝 Selection Sync 实现日志

**日期**：2026-01-16  
**分支**：`feat/selectionAPI`  
**PR**：#7 - feat/selection api: add support to selecting note/bar and highlight its alphaTex in editor

---

## 🎯 目标

实现 alphaTab 乐谱与 CodeMirror 编辑器之间的**双向选区同步**：

1. **乐谱 → 编辑器**：在乐谱上选择音符/小节时，编辑器中对应的 AlphaTex 代码高亮
2. **编辑器 → 乐谱**：点击编辑器代码时，乐谱滚动并高亮到对应位置
3. **播放跟随高亮**：播放时，编辑器实时高亮当前播放的音符（绿色）

---

## 📁 涉及文件

| 文件                                          | 职责                                                       |
| --------------------------------------------- | ---------------------------------------------------------- |
| `src/renderer/store/appStore.ts`              | 新增 `scoreSelection`, `editorCursor`, `playbackBeat` 状态 |
| `src/renderer/lib/alphatex-selection-sync.ts` | 核心同步逻辑：解析器、高亮扩展、位置映射                   |
| `src/renderer/components/Preview.tsx`         | 监听 alphaTab 事件，更新 store                             |
| `src/renderer/components/Editor.tsx`          | 监听 store 变化，更新 CodeMirror 高亮                      |
| `src/renderer/lib/alphatex-barlines.ts`       | 修复并发装饰更新导致的 CodeMirror 崩溃                     |

---

## 🏗️ 架构设计

```
┌─────────────────────────────────────────────────────────────────────┐
│                           Zustand Store                              │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐      │
│  │ scoreSelection  │  │  editorCursor   │  │  playbackBeat   │      │
│  │ (乐谱选区)      │  │ (编辑器光标)    │  │ (播放位置)      │      │
│  └────────┬────────┘  └────────┬────────┘  └────────┬────────┘      │
└───────────┼────────────────────┼────────────────────┼───────────────┘
            │                    │                    │
            ▼                    ▼                    ▼
┌───────────────────┐  ┌───────────────────┐  ┌───────────────────┐
│    Preview.tsx    │  │    Editor.tsx     │  │    Preview.tsx    │
│ playbackRange     │  │ cursor tracking   │  │ playedBeatChanged │
│ HighlightChanged  │  │ extension         │  │ event             │
└───────────────────┘  └───────────────────┘  └───────────────────┘
            │                    │                    │
            ▼                    ▼                    ▼
┌───────────────────┐  ┌───────────────────┐  ┌───────────────────┐
│  Editor 蓝色高亮   │  │  Preview 高亮     │  │  Editor 绿色高亮   │
│  (选区同步)        │  │  (反向同步)       │  │  (播放跟随)        │
└───────────────────┘  └───────────────────┘  └───────────────────┘
```

---

## 🔧 核心实现

### 1. AlphaTex 解析器 (`parseBeatPositions`)

将 AlphaTex 源码解析为 Beat 位置映射：

```typescript
interface BeatCodePosition {
  barIndex: number; // 小节索引 (0-based)
  beatIndex: number; // Beat 在小节内的索引
  startOffset: number; // 代码起始位置
  endOffset: number; // 代码结束位置
  // ... 行列信息
}
```

**关键逻辑**：

- 从 `.` 标记开始解析（跳过元数据）
- 和弦 `(...)` 作为整体处理
- 时值修饰符 `:8` 等不计入 beat 索引

### 2. CodeMirror 装饰扩展

两种高亮样式：

- **选区高亮** (`cm-score-selection-highlight`)：蓝色/紫色，手动选择触发
- **播放高亮** (`cm-playback-highlight`)：绿色，播放时自动跟随

```typescript
export function createSelectionSyncExtension(): Extension[];
export function createPlaybackSyncExtension(): Extension[];
```

### 3. 反向同步 (编辑器 → 乐谱)

```typescript
// 光标位置追踪
createCursorTrackingExtension((cursor) => {
  setEditorCursor(cursor); // 更新 store
});

// Preview 监听 editorCursor 变化
useEffect(() => {
  const beat = findBeatInScore(
    score,
    editorCursor.barIndex,
    editorCursor.beatIndex
  );
  api.highlightPlaybackRange(beat, beat);
}, [editorCursor]);
```

---

## 🐛 Bug 修复记录

### Bug 1: 时值修饰符被错误识别为 Beat

**现象**：`:8 0.6 2.5` 中，`:8` 被识别为第一个 beat

**原因**：解析器将所有空格分隔的 token 都当作 beat

**修复**：添加 `isNonBeatToken()` 函数识别并跳过时值修饰符

```typescript
// 纯时值修饰符，跳过
:8, :4, :16, :4., :8{tu 3}

// 包含音符，不跳过
:8(3.2 0.3), :83.2
```

### Bug 2: 和弦被拆分为多个 Beat

**现象**：`(0.1 0.6)` 被拆分为 `(0.1` 和 `0.6)`

**原因**：括号内的空格被当作分隔符

**修复**：添加 `inChord` 状态和 `chordDepth` 深度追踪

```typescript
if (char === "(") {
  inChord = true;
  chordDepth++;
}
// 和弦内的空格不作为分隔符
if (inChord) continue;
```

### Bug 3: 紧凑写法解析失败

**现象**：`:8(3.2 0.3)` 整个被忽略（无空格分隔时值和和弦）

**原因**：`isNonBeatToken` 检测到 `:数字` 开头就跳过整个 token

**修复**：添加 `extractBeatContent()` 函数分离时值前缀

```typescript
function extractBeatContent(token: string) {
  // :8(3.2 0.3) → { content: "(3.2 0.3)", prefixLength: 2 }
}
```

### Bug 4: 元数据被识别为音符

**现象**：`\tab "author"` 中的字符串被识别为 beat

**原因**：解析器没有正确识别 `.` 作为内容起始标记

**修复**：优先查找单独的 `.` 作为内容起始点

```typescript
// 查找单独的 "."（不是小数点）
if (char === "." && !isPrevDigit && !isNextDigit) {
  contentStart = i + 1;
  foundDot = true;
  break;
}
```

### Bug 5: CodeMirror 并发装饰更新崩溃

**现象**：滚动时出现 `Cannot destructure property 'tile' of undefined`

**原因**：多个 StateField 同时 dispatch effect 导致内部状态不一致

**修复**：

1. 使用 `requestAnimationFrame` 延迟 dispatch
2. 全面的 try-catch 保护
3. dispatch 前检查 view 状态

```typescript
function safeDispatch(view: EditorView, effect: StateEffect<...>) {
  if (!view?.dom || !document.contains(view.dom)) return;
  requestAnimationFrame(() => {
    try {
      view.dispatch({ effects: effect });
    } catch (err) {
      console.error("[SelectionSync] Failed to dispatch:", err);
    }
  });
}
```

---

## 📊 测试用例

| 输入                 | 期望 Bar:Beat | 说明                       |
| -------------------- | ------------- | -------------------------- |
| `0.6 2.5 3.2 \|`     | 0:0, 0:1, 0:2 | 基础音符                   |
| `:8 0.6 2.5 \|`      | 0:0, 0:1      | 时值修饰符被跳过           |
| `(0.1 0.6) 2.5 \|`   | 0:0, 0:1      | 和弦作为整体               |
| `:8(3.2 0.3) 2.5 \|` | 0:0, 0:1      | 紧凑写法，高亮 `(3.2 0.3)` |
| `r.4 0.6 \|`         | 0:0, 0:1      | 休止符是有效 beat          |

---

## 🎨 视觉效果

### 高亮样式

```css
/* 选区高亮 - 蓝色 */
.cm-score-selection-highlight {
  background-color: hsl(var(--primary) / 0.25);
  box-shadow: 0 0 0 1px hsl(var(--primary) / 0.4);
}

/* 播放高亮 - 绿色 */
.cm-playback-highlight {
  background-color: hsl(142 76% 36% / 0.3);
  box-shadow: 0 0 0 1px hsl(142 76% 36% / 0.5);
}
```

---

## 📝 后续优化方向

1. **性能优化**：对于大型文件，考虑增量解析而非全量解析
2. **更精确的位置映射**：支持 Note 级别（当前是 Beat 级别）
3. **多音轨支持**：当前只处理第一个音轨
4. **错误容错**：解析失败时的优雅降级

---

## 📚 参考资料

- [alphaTab 1.8.0 Selection API](./SelectionAPI.md)
- [CodeMirror 6 Decorations](https://codemirror.net/docs/ref/#view.Decoration)
- [Zustand State Management](https://github.com/pmndrs/zustand)
